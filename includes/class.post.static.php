...
class ezTOC_Post_Static {

	/**
	 * @since 2.0
	 * @var int
	 */
	private static $queriedObjectID;

        public static $the_content;
	/**
	 * @since 2.0
	 * @var WP_Post
	 */
	 ...
	 * @var array
	 */
	private static $headingLevels = array();
	 ...
	 * @var string[]
	 */
	private static $excludedNodes = array();
	 ...
	 * @var bool
	 */
	private static $hasTOCItems = false;

                                
        public static function init(WP_Post $post, $content = '' ) {

		self::$post            = $post;
		self::$permalink       = get_permalink( $post );
		self::$queriedObjectID = get_queried_object_id();
                
                $theContent = empty($content) ? static::$the_content : $content;
                
                self::process($theContent);
        }
	 ...
	 * @return static
	 */
	private static function process( $content ) {

		self::processPages( $content );
	 ...
	 * @return int
	 */
	protected static function getNumberOfPages() {
	 ...
	 * @return bool
	 */
	protected static function isMultipage() {

		return 1 < self::getNumberOfPages();
	}
	 ...
	 * @since  2.0
	 */
	private static function processPages($content) {
...
//				$content .= get_option( 'ez-toc-post-meta-content' )[ get_the_ID() ];
//			}
//		} else if ( ( ezTOC::isCoreLevel() ) && false != get_option( 'ez-toc-post-content-core-level' ) ) {
...
//                }
		...
		if ( is_array( $split ) ) {
			...
			foreach ( $split as $content ) {
				...
				self::extractExcludedNodes( $page, $content );

				$pages[ $page ] = array(
					'headings' => self::extractHeadings( $content ),
					'content'  => $content,
				);
	 ...
	 */
	private static function extractExcludedNodes( $page, $content ) {
		...
		if(isset($nodes['ids'])){
			foreach ( $nodes['ids'] as $id ) {
				...
				//self::$excludedNodes[ $page ][ $id ] = $html->Implode( $id, $tagFilterOptions );
				array_push( self::$excludedNodes, $html->Implode( $id, $tagFilterOptions ) );
			}
		}
		//$eligibleContent = $html->Implode( 0, $tagFilterOptions );
	...
	private static function extractHeadings( $content ) {
		...
		//static::$collision_collector = array();

		if ( in_array( 'elementor/elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || ezTOC::isCoreLevel() ) {
		...
		if ( preg_match_all( '/(<h([1-6]{1})[^>]*>)(.*)<\/h\2>/msuU', $content, $matches, PREG_SET_ORDER ) ) {
			...
			self::removeHeadingsFromExcludedNodes( $matches );
			self::removeHeadings( $matches );
			self::excludeHeadings( $matches );
			self::removeEmptyHeadings( $matches );

			if ( count( $matches ) >= $minimum ) {

				self::alternateHeadings( $matches );
				self::headingIDs( $matches );
				self::$hasTOCItems = true;
	 ...
	 * @return bool
	 */
	private static function inExcludedNode( $string ) {

		foreach ( self::$excludedNodes as $node ) {
	 ...
	 * @return array
	 */
	private static function removeHeadingsFromExcludedNodes( &$matches ) {

		foreach ( $matches as $i => $match ) {

			if ( self::inExcludedNode( "{$match[3]}</h$match[2]>" ) ) {
	 ...
	 * @return array
	 */
	private static function getHeadingLevels() {

		$levels = get_post_meta( self::$post->ID, '_ez-toc-heading-levels', true );
		...
		self::$headingLevels = $levels;

		return self::$headingLevels;
	}
	 ...
	 * @return array
	 */
	private static function removeHeadings( &$matches ) {

		$levels = self::getHeadingLevels();
	 ...
	 * @return array
	 */
	private static function excludeHeadings( &$matches ) {

		$exclude = get_post_meta( self::$post->ID, '_ez-toc-exclude', true );
				...
				foreach ( $matches as $i => $match ) {
					...
					for ( $j = 0; $j < $excluded_count; $j++ ) {
						...
						$pattern = html_entity_decode(
							( ezTOC::isCoreLevel() ) ? $excluded_headings[ $j ] : wptexturize($excluded_headings[ $j ]),
							ENT_NOQUOTES,
							get_option( 'blog_charset' )
						);
	 ...
	 * @return array
	 */
	private static function getAlternateHeadings() {

		$alternates = array();
		$value      = get_post_meta( self::$post->ID, '_ez-toc-alttext', true );
	 ...
	 * @return array
	 */
	private static function alternateHeadings( &$matches ) {

		$alt_headings = self::getAlternateHeadings();
		//$count        = count( $matches );
	 ...
	 * @return mixed
	 */
	private static function headingIDs( &$matches ) {
		...
		foreach ( $matches as $i => $match ) {
			...
			$matches[ $i ]['id'] = self::generateHeadingIDFromTitle( $matches[ $i ][0] );
		}
	 ...
	 * @return bool|string
	 */
	private static function generateHeadingIDFromTitle( $heading ) {
	 ...
	 * @return array
	 */
	private static function removeEmptyHeadings( &$matches ) {
	 ...
	 * @return bool
	 */
	public static function hasTOCItems() {

		return self::$hasTOCItems;
	}
	 ...
	 * @return array
	 */
	public static function getHeadings( $page = null ) {
		...
		if ( isset( self::$pages[ $page ] ) ) {
			...
			//$headings = wp_list_pluck( self::$pages[ $page ]['headings'], 0 );

			$matches = self::$pages[ $page ]['headings'];
			//$count   = count( $matches );
	 ...
	 * @return array
	 */
	public static function getHeadingsWithAnchors( $page = null ) {
	 ...
	 * @return string
	 */
	public static function getTOCList($prefix = "ez-toc") {

		$html = '';

		if ( self::$hasTOCItems ) {

			foreach ( self::$pages as $page => $attribute ) {

				$html .= self::createTOC( $page, $attribute['headings'], $prefix );
			}
	 ...
	 *
	 */
	public static function getStickyToggleTOC() {
...
//				}
			...
			$htmlSticky .= "<nav class='$ezTocStickyToggleDirection'>" . self::getTOCList( "ez-toc-sticky" ) . "</nav>";
			ob_start();
	 ...
	 */
	private static function createTOC( $page, $matches, $prefix = "ez-toc" ) {
		...
		if ( $hierarchical ) {
			...
			foreach ( $matches as $i => $match ) {
				...
				$html .= self::createTOCItemAnchor( $page, $matches[ $i ]['id'], $title, $count );
	 ...
	 * @return string
	 */
	private static function createTOCItemAnchor( $page, $id, $title, $count ) {
		...
		return sprintf(
			...
			esc_attr( self::createTOCItemURL( $id, $page ) ),
	 ...
	 * @return string
	 */
	private static function createTOCItemURL( $id, $page ) {

		$current_post = self::$post->ID === self::$queriedObjectID;
		...
		if ( $page === $current_page && $current_post ) {
			...
			return trailingslashit( self::$permalink ) . '#' . $id;

		}

		return trailingslashit( self::$permalink ) . $page . '/#' . $id;
	}
}
