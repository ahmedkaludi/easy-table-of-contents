...
/**
 * GP Premium Element Compatiblity
 *
 * @since 2.0.50
 */
if ( in_array( 'gp-premium/gp-premium.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) 
{
	add_filter( 'ez_toc_modify_process_page_content', 'ez_toc_gn_premium_element_compatibility' );
	/**
	 * ez_toc_gn_premium_element_compatibility function
	 *
	 * @since 2.0.50
	 * @param string|mixed $content
	 * @return string|mixed
	 */
	function ez_toc_gn_premium_element_compatibility( $content )
	{
		$args = array(
		...
		if( null !== $posts && !empty( $posts ) ) 
		{
			...
			{
				$_generate_element_display_conditions = get_post_meta( $postID, '_generate_element_display_conditions', true );
				if( null !== $_generate_element_display_conditions && !empty( $_generate_element_display_conditions ) )
				{
					foreach($_generate_element_display_conditions as $_generate_element_display_condition)
					{
						if( key_exists( 'object', $_generate_element_display_condition ) )
						{
							if( $_generate_element_display_condition['object'] == get_the_ID() )
							{
	...
	add_filter( 'generate_do_block_element_content', array( 'ezTOC', 'the_content' ), 100 );
}