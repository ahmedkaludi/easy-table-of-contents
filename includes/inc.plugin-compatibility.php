	...
	function() {
		...
		if ( function_exists( 'uncode_setup' ) ) {
			...
			add_filter(
				...
				function( $content ) {
					return ( in_array( 'divi-machine/divi-machine.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) || 'Fortunato Pro' == apply_filters( 'current_theme', get_option( 'current_theme' ) ) ) || 'Pale Moon' == get_browser_name() ) ? $content : wptexturize($content);
				},
				10,
				1
			);
